/// @description TextField - Draw
if (!surface_exists(Surface) || Text_is_dirty) {
  draw_set_font(fnt_game);
  
  var sw = (string_width(string_hash_to_newline("W")) * Text_width), sh = string_height(string_hash_to_newline("W")); 
  var fw = sw + 8, fh = sh + 6;
  var tx = 4, ty = 5;
...
if (global.__field_focus == id) {
  ...
  if (time % 2 == 0) {
    var cx = 2 + string_width(string_hash_to_newline(Text)) + 2;
    var cy = 2; ch = string_height(string_hash_to_newline("W")) + 1;
